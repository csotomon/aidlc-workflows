...
1. **Invoke reviewer sub-agent.** Delegate to the reviewer agent named in `directive.reviewer`. Pass:
   ...
   **Record the dispatch.** As you invoke the reviewer, record it: `bun .kiro/tools/aidlc-log.ts review --stage <slug> --reviewer <directive.reviewer> --iteration <n>` (add `--unit <directive.unit>` on a per-unit stage). This emits `REVIEW_REQUESTED` so a dispatched-but-never-completed review is visible in the audit trail.
...
3. **Read verdict.** After the reviewer returns, delete `<record>/.aidlc-reviewer-dispatch.json` if one was written (the enforcement window closes with the review; a leftover record would keep refusing sibling access for later, unrelated work), then read the `## Review` section from the primary artifact. Then **record the outcome with the tool actor** so the review is observable and the gate can enforce it:
   ...
   ```
   bun .kiro/tools/aidlc-log.ts review --stage <slug> --reviewer <directive.reviewer> --iteration <n> --verdict <READY|NOT-READY>
   ```
