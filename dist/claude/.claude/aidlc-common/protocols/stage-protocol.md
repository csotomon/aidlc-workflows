...
1. **Invoke reviewer sub-agent.** Delegate to the reviewer agent named in `directive.reviewer`. Pass:
   ...
   **Record the dispatch.** As you invoke the reviewer, record it: `bun .claude/tools/aidlc-log.ts review --stage <slug> --reviewer <directive.reviewer> --iteration <n>` (add `--unit <directive.unit>` on a per-unit stage). This emits `REVIEW_REQUESTED` so a dispatched-but-never-completed review is visible in the audit trail.
